...
function zf_font_url() {
	 ...
	 */
	$roboto = _x( 'on', 'Roboto font: on or off', 'zf-theme' );
	$open_sans = _x( 'on', 'Open Sans font: on or off', 'zf-theme' );
...
function zf_scripts() {
	...
	// Register styles.
	wp_register_style( 'zf-theme-google-font', zf_font_url(), array(), null );

	// Enqueue styles.
	wp_enqueue_style( 'zf-theme-google-font' );
...
add_action( 'wp_enqueue_scripts', 'zf_scripts' );
...
if ( class_exists( 'WDS_Simple_Page_Builder' ) && version_compare( WDS_Simple_Page_Builder::VERSION, '1.6', '>=' ) ) :
	 ...
	 */
	function zf_enqueue_page_builder_scripts() {
	...
	add_action( 'wds_page_builder_after_load_parts', 'zf_enqueue_page_builder_scripts' );
